                                      ...
                                      const ISavestate& save)
{
  ...
  {
    std::ostringstream pathStream(MakePath(gamePath));
    pathStream << URIUtils::AppendSlash(pathStream.str());
    pathStream << CDateTime::GetCurrentDateTime().GetAsSaveString() << SAVESTATE_EXTENSION;
